...
# Precompute and factorize the three LHS matrices (BCs fixed, so do this once)
...
for n in range(1, n_steps + 1):
    ...
    if n == 1:
        ...
        RHS[0] = np.sin(np.pi * t_next);  RHS[-1] = 0.0
        U_new = solve1(RHS)
    elif n == 2:
...
plt.figure()
plt.plot(nodes, U_prev1, 'b-', linewidth=1.5)
plt.title('Unsteady Advection-Diffusion at t=1.5 (BDF3/EXT3)')
plt.xlabel('x')
...
def update(i):
    ...
    title.set_text(f'Unsteady Advection-Diffusion  t = {frames_t[i]:.3f}')
    return line, title
...
ani.save('2a.gif', writer='pillow', fps=20)
plt.close()
print('Saved 2a.gif')